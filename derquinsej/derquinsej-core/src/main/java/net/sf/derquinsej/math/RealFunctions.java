...
public final class RealFunctions {
	...
	/**
	 * Transform a real function into a function that does not allow the null
	 * input.
	...
	/**
	 * Returns a partial real function that uses linear interpolation based on
	 * table of values. The provided table of values is an array with an even
	 * number of elements in the form {x1, f(x1), x2, f(x2), ..., xn, f(xn)}
	 * with x1 ... xn in ascending order, and at least two pairs must be
	 * provided. The provided table is not defensively copied so it MUST NOT be
	 * modified. No value outside the table is extrapolated.
	 * @return The requested function.
	 * @throws NullPointerException if the argument is {@code null}.
