...
/**
 * Partial real function that uses linear interpolation based on table of
 * values. The provided table of values is an array with an even number of
 * elements in the form {x1, f(x1), x2, f(x2), ..., xn, f(xn)} with x1 ... xn in
 * ascending order, and at least two pairs must be provided. The provided table
 * is not defensively copied so it MUST NOT be modified. No value outside the
...
class LerpRealFunction implements PartialRealFunction {
	...
	public String toString() {
		return String.format((Locale)null, "Linear interpolation for inputs in [%f, %f]", table[0], table[table.length-1]);
	}
}
