...
abstract class LongPopulationImpl implements LongPopulation {
	...
	public String toString() {
		return String.format((Locale) null, "[n=%d, mu=%f, min=%d, max=%d, s=%f, s2=%f]", getCount(), getMean(),
				getMin(), getMax(), getSigma(), getVariance());
	}
	...
	private static final class Many extends LongPopulationImpl {
		...
		@Override
		public int hashCode() {
			return new HashBuilder().add(n).add(min).add(max).add(mean).add(m2).hashCode();
		}
